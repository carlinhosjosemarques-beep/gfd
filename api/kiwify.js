...
function pickSubscriptionId(body) {
  return String(
...
function inferPaidBlocked(event, status, paymentMethod) {
  const norm = `${event} ${status} ${paymentMethod}`.toLowerCase();
...
function normalizeSupabaseUrl(urlRaw) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { isPaid, isBlocked, norm } = inferPaidBlocked(event, status, paymentMethod);

    if (debug) {
      console.log("[KIWIFY] email:", emailNorm);
      console.log("[KIWIFY] orderId:", orderId);
      console.log("[KIWIFY] subscriptionId:", subscriptionId);
